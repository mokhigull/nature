...
SCORE_BANDS: list = [
    (0,  15, "🌿 Excellent Nature Connection",
    ...
    (16, 30, "🌱 Good Restoration Habits",
     ...
     "good"),
    (31, 45, "☁️ Moderate Nature Engagement",
    ...
    (61, 75, "😔 Minimal Restoration",
     ...
     "minimal"),
    (76, 90, "⚠️ Poor Clarity Restoration",
     ...
     "poor"),
    (91, 100, "🚨 Critical Disconnection from Nature",
...
def page_load():
    ...
    col_search, col_upload = st.columns([1.2, 1])
    ...
    with col_search:
        ...
        sid_input: str = st.text_input("Student ID", placeholder="e.g. 12345678",
                                        key="load_sid")

        if st.button("Search Results", type="primary", use_container_width=True):
            if not validate_student_id(sid_input.strip()):
                st.error("Please enter a valid Student ID (digits only).")
            else:
                ...
                if record:
                    ...
                    <div class="score-box" style="margin-top:16px;">
                        <div class="score-number">{score_int}
                            <span style="font-size:0.5em; font-weight:400;
                    ...
                    <div class="card-accent" style="margin-top:12px;">
                        ...
                        <strong>🕐 Date Taken:</strong> {record.get('date_taken','—')}<br>
                        <strong>🎓 Student ID:</strong> {record.get('student_id','—')}
                    </div>
                    """, unsafe_allow_html=True)

                    st.markdown(f"""
                    <div class="card" style="margin-top:10px;">
                        <p style="color:{t['text']}; line-height:1.7;">{desc}</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.error(
    ...
    with col_upload:
        st.markdown(f'<div class="card">', unsafe_allow_html=True)
        st.markdown("### 📤 Upload a Saved File")
        st.markdown(
            "Alternatively, upload a previously downloaded results file "
            "(JSON, CSV, or TXT)."
        )
